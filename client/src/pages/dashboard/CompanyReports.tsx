...
import { Globe, Mail, Linkedin, Twitter, Flame, ArrowUpDown, Loader2 } from "lucide-react";
...
export default function CompanyReports() {
  ...
  const { role: rawRole, plan: rawPlan, loading: accessLoading } = useUserAccess(user, authLoading);

  const role = rawRole?.toLowerCase() ?? "guest";
  const plan = rawPlan?.toLowerCase() ?? "free";
  const hardLimit = PLAN_LIMITS[role === "guest" ? "guest" : plan] ?? PLAN_LIMITS["free"];
  ...
  const { data: companies, isLoading } = useQuery({
    queryKey: ["funded-companies", role, plan],
    enabled: !authLoading && !accessLoading && !!user,
    queryFn: async () => {
      let query = supabase
  ...
  const analytics = useMemo(() => {
    ...
    return {
      newThisWeek: companies.filter((c) => new Date(c.created_at) >= weekAgo).length,
      total: companies.length,
      hotLeads: companies.filter(isHotLeadCompany).length,
      totalContacts: companies.reduce((acc, c) => acc + (c.contacts?.length || 0), 0),
    };
  }, [companies]);
  ...
  const filteredAndSortedCompanies = useMemo(() => {
    ...
    filtered.sort((a, b) => {
      let comparison = 0;
      if (sortBy === "date") comparison = (new Date(a.funding_date || 0).getTime() - new Date(b.funding_date || 0).getTime());
      else if (sortBy === "amount") comparison = (a.funding_amount || 0) - (b.funding_amount || 0);
      else comparison = a.company_name.localeCompare(b.company_name);
  ...
  const totalPages = useMemo(() => Math.max(1, Math.ceil(filteredAndSortedCompanies.length / PAGE_SIZE)), [filteredAndSortedCompanies]);
  ...
  const getStageBadgeVariant = (stage: string | null) => {
    ...
    if (lower.includes("series a") || lower.includes("series b") || lower.includes("series c")) return "default";
    return "outline";
  };
    ...
    <div className="relative min-h-screen p-8 bg-gray-50 text-gray-900">
      ...
      <Card className="p-6 mb-6 border border-gray-200 shadow-lg rounded-2xl bg-white">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          <Input placeholder="Search companies, industries..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="bg-gray-100 text-gray-900 placeholder-gray-500" />
                ...
                <Fragment key={company.id}>
                  <tr className={`${isExpanded ? "bg-gray-50" : ""}`}>
                    ...
                    <td className="px-4 py-2">
                      {company.funding_stage && <Badge variant={getStageBadgeVariant(company.funding_stage)}>{company.funding_stage}</Badge>}
                    </td>
                    ...
                    <td className="px-4 py-2">
                      ...
                      {company.investors && company.investors.length > 2 ? ` +${company.investors.length - 2} more` : ""}
                    </td>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          ...
                          {company.domain && (
                            <div className="flex items-center gap-2 mt-2">
                              ...
                              <a href={`https://${company.domain}`} target="_blank" rel="noopener noreferrer" className="underline text-blue-600">{company.domain}</a>
                            </div>
                          )}
                        </div>
                      </td>
